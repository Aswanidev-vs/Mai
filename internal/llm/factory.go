...
func (f *Factory) TestCloudProvider() error {
	...
	log.Printf("[TEST] Provider: %s, Model: %s, URL: %s", provider, model, url)
...
func (f *Factory) CreateHybridProvider() (interfaces.LLMProvider, error) {
	...
	local := NewOllamaProvider(localModel, localURL, f.config.LLM.SystemPrompt, f.config.LLM.Think, f.ollamaOpts())
...
func (f *Factory) createCloudProvider(provider, model, url, apiKey string) (interfaces.LLMProvider, error) {
	...
	case "ollama":
		...
		return NewOllamaProvider(model, url, f.config.LLM.SystemPrompt, f.config.LLM.Think, f.ollamaOpts()), nil
	case "openai":
		if url == "" {
...
func (f *Factory) CreateProvider(providerType string) (interfaces.LLMProvider, error) {
	switch providerType {
	case "ollama":
		return NewOllamaProvider(f.config.LLM.Model, f.config.LLM.URL, f.config.LLM.SystemPrompt, f.config.LLM.Think, f.ollamaOpts()), nil
